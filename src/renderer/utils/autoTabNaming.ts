...
export function maybeStartAutomaticTabNaming({
	...
	const hasCustomName =
		!!activeTab?.name && !isAutoGeneratedThreadName(activeTab.name, session.toolType);
	if (!activeTab || activeTab.agentSessionId || hasCustomName) {
		return;
	}
		...
		.then((generatedName) => {
			...
			setSessions((prev) =>
				prev.map((candidateSession) => {
					...
					const targetTabHasCustomName =
						!!targetTab?.name &&
						!isAutoGeneratedThreadName(targetTab.name, candidateSession.toolType);
					if (!targetTab || targetTabHasCustomName) {
