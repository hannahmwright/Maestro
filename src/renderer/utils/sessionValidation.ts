 ...
 *
 * Rules:
 * 1. Session names must be unique per provider across all sessions (hard error)
 * 2. Home directories (projectRoot) shared with any existing thread runtime produce a warning
...
export function validateNewSession(
	...
	// Check for duplicate visible name within the same provider.
	// The internal uniqueness key is effectively name + provider.
	...
	if (duplicateName) {
		const providerName = getProviderDisplayName(toolType);
		return {
			valid: false,
			error: `A ${providerName} thread named "${duplicateName.name}" already exists`,
			errorField: 'name',
		};
	}
	...
	if (conflictingAgents.length > 0) {
		...
		return {
			...
			warning: `This workspace directory is already used by ${agentList}. Running multiple thread runtimes in the same directory may cause them to clobber each other's work.`,
			warningField: 'directory',
...
export function getDefaultThreadNameBase(toolType: ToolType): string {
	return `${getProviderDisplayName(toolType)} Thread`;
}

export function buildDefaultThreadName(toolType: ToolType, existingSessions: Session[]): string {
	const baseName = getDefaultThreadNameBase(toolType);
	const existingNames = new Set(
		existingSessions
			.filter((session) => session.toolType === toolType)
...
export function isAutoGeneratedThreadName(name: string | null | undefined, toolType: ToolType): boolean {
	const trimmedName = name?.trim();
	if (!trimmedName) return false;

	const baseName = getDefaultThreadNameBase(toolType);
	const escapedBaseName = baseName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
	return new RegExp(`^${escapedBaseName}(?: \\d+)?$`, 'i').test(trimmedName);
}
 ...
 *
 * Rules:
 * 1. Session names must be unique per provider across all sessions (excluding the current session)
 */
export function validateEditSession(
	...
	// Check for duplicate visible name within the same provider (excluding the current session).
	const duplicateName = existingSessions.find(
		(session) =>
