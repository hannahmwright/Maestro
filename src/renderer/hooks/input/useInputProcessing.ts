...
import { isAutoGeneratedThreadName } from '../../utils/sessionValidation';
...
export function useInputProcessing(deps: UseInputProcessingDeps): UseInputProcessingReturn {
	...
	const processInput = useCallback(
		async (overrideInputValue?: string, options?: { disposition?: 'default' | 'queue' }) => {
			...
			if (currentMode === 'ai') {
				...
				const demoCapture: DemoCaptureRequest | undefined = activeTab?.demoCaptureRequested
					? { enabled: true }
					: undefined;
				const hasDraftContent = effectiveInputValue.trim().length > 0 || stagedImages.length > 0;
			...
			const hasNoCustomName =
				!activeTabForNaming?.name ||
				isAutoGeneratedThreadName(activeTabForNaming.name, activeSession.toolType);
