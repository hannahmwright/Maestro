...
function RunningSpinner({ theme }: { theme: Theme }) {
	return (
		<span
			aria-hidden="true"
			style={{
				...
				height: '12px',
				borderRadius: '999px',
				border: `1.6px solid ${theme.colors.warning}35`,
...
function FailedMarker({ theme }: { theme: Theme }) {
	return (
		<span
			aria-hidden="true"
			style={{
...
export const ToolActivityPanel = memo(function ToolActivityPanel({
	logs,
	theme,
	isSessionBusy = false,
		...
		() =>
			logs
				.map((log) => {
					...
					return `${log.id || log.timestamp}:${status}:${startedAt}:${completedAt}:${log.text || ''}`;
				})
				.join('|'),
		[logs]
	);
	...
	if (!isSessionBusy && !isPanelExpanded) {
		return (
			...
			>
				...
				<ChevronDown size={14} color={theme.colors.textDim} />
			</button>
		);
	}
		...
		>
			...
			{!isSessionBusy && (
				...
				>
					...
					<ChevronUp size={14} color={theme.colors.textDim} />
				</button>
			)}
			...
			<div
				style={{
					position: 'relative',
					borderRadius: '16px',
					background: `linear-gradient(180deg, ${theme.colors.bgSidebar}b8 0%, ${theme.colors.bgMain}c8 100%)`,
					overflow: 'hidden',
				}}
			>
				<div
						...
						(
							...
							>
								...
								>
									...
									>
										{status === 'running' ? (
											<RunningSpinner theme={theme} />
										) : status === 'error' ? (
											<FailedMarker theme={theme} />
										) : null}
									</span>
									<div
										style={{
											...
											>
												...
												<div
													style={{
														padding: '8px 10px',
														borderRadius: '12px',
														background: `${theme.colors.bgSidebar}b8`,
														fontSize: '11px',
														fontFamily:
												...
												{sourceDomains.map((domain) => (
													<span
														...
														style={{
															...
															background: `${theme.colors.bgSidebar}bb`,
															fontSize: '10px',
															color: theme.colors.textDim,
														}}
													>
				...
				{pendingCount > 0 && !isAtBottom && (
					<button
						...
						style={{
							...
							borderRadius: '999px',
							border: `1px solid ${theme.colors.border}`,
							background: `${theme.colors.bgMain}f5`,
